...
class MockSystem implements StatusLightsSystem
{
    ...
    {
        return substr($this->input, 0, $maxBytes);
    }
...
final class StatusLightsTest extends TestCase
{
    ...
    public function testResolvesWorkflowSuccessfully(): void
    {
        status_lights_app_write(
            ...
            status_lights_app_repo_key('owner', 'repo'),
            ['installed' => true],
        );
        ...
        status_lights_app_write(
            ...
            status_lights_app_status_key('owner', 'repo', 'ci.yml'),
            ['state' => 'success', 'updated_at' => 1000],
        );

        $response = status_lights_app_handle_request(
            $this->mock,
            ['REQUEST_URI' => '/github/owner/repo/ci.yml.svg'],
        );
    ...
    public function testStatusLightsRealSystemIntegration(): void
    {
        ...
        $this->assertFalse($real->createAtomicFile($temp, 'fail'));
