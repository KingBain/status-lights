...
{
    if (preg_match($kind->keyPattern(), $key) !== 1) throw new InvalidArgumentException('Invalid app data record key.');
...
function status_lights_app_write(StatusLightsSystem $system, StatusLightsAppStoreKind $kind, string $key, array $value): void
{
    ...
    if (!is_string($temporary)) throw new RuntimeException('Unable to create temporary app data file.');
...
function status_lights_app_create(StatusLightsSystem $system, StatusLightsAppStoreKind $kind, string $key, array $value): bool
{
    ...
    if ($result === null) throw new RuntimeException('Unable to create app data record.');
...
function status_lights_app_prune_records_older_than(StatusLightsAppStoreKind $kind, int $cutoff, ?StatusLightsSystem $system = null): int
{
    ...
    $directory = status_lights_app_store_kind_directory($system, $kind);
    if (!$system->isDir($directory)) return 0;

    $deleted = 0;
    foreach ($system->getJsonFilesInDirectory($directory) as $pathname => $basename) {
        if (preg_match($kind->keyPattern(), $basename) !== 1) continue;
...
function status_lights_app_read_webhook_body(StatusLightsSystem $system, array $server): string
{
    ...
    if (strlen($body) > $maximumBytes) throw new StatusLightsPayloadTooLargeException('Webhook payload is too large.');
    
    // We expect valid JSON, so an empty body on a POST is fundamentally unreadable/invalid
    if ($body === '' && ($server['REQUEST_METHOD'] ?? '') === 'POST') throw new RuntimeException('Unable to read webhook payload.');
    return $body;
}

function status_lights_app_repo_key(string $owner, string $repository): string { return status_lights_app_key(strtolower($owner), strtolower($repository)); }
function status_lights_app_status_key(string $owner, string $repository, string $workflow, ?string $job = null): string
{
    ...
    if ($job !== null) { array_push($parts, 'job', $job); }
    return status_lights_app_key(...$parts);
}

function status_lights_app_normalize_delivery_id(mixed $value): ?string
{
    if (!is_string($value) || preg_match('/\A[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\z/iD', $value) !== 1) return null;
...
function status_lights_app_delivery_key(string $deliveryId): string
{
    ...
    if ($normalized === null) throw new InvalidArgumentException('Invalid GitHub webhook delivery ID.');
...
function status_lights_app_verify_signature(StatusLightsSystem $system, string $body, array $server): bool
{
    ...
    if ($secret === '' || !is_string($signature) || !str_starts_with($signature, 'sha256=')) return false;
...
function status_lights_app_mark_repository(StatusLightsSystem $system, array $repository, ?int $installationId, bool $installed): void
{
    ...
    if (!is_string($owner) || !is_string($name)) return;
...
function status_lights_app_handle_installation(StatusLightsSystem $system, string $event, array $payload): void
{
    ...
    if ($event === 'installation') {
        ...
        if (is_array($repositories)) {
            foreach ($repositories as $repository) {
                if (is_array($repository)) status_lights_app_mark_repository($system, $repository, $installationId, $installed);
            }
        }
        return;
    }
    ...
    if ($event === 'installation_repositories') {
        foreach (($payload['repositories_added'] ?? []) as $repository) {
            if (is_array($repository)) status_lights_app_mark_repository($system, $repository, $installationId, true);
        }
        foreach (($payload['repositories_removed'] ?? []) as $repository) {
            if (is_array($repository)) status_lights_app_mark_repository($system, $repository, $installationId, false);
        }
    }
}
...
function status_lights_app_handle_workflow_run(StatusLightsSystem $system, array $payload): void
{
    ...
    if (!is_array($repository) || !is_array($run)) return;
    ...
    if (!is_string($owner) || !is_string($name) || !is_string($path) || !is_int($runId) || $runId < 1) return;
    if (is_string($defaultBranch) && is_string($headBranch) && $headBranch !== $defaultBranch) return;
...
function status_lights_app_handle_workflow_job(StatusLightsSystem $system, array $payload): void
{
    ...
    if (!is_array($repository) || !is_array($job)) return;
    ...
    if (!is_string($owner) || !is_string($name) || !is_string($jobName) || !is_int($runId) || $runId < 1) return;
    if (is_string($defaultBranch) && is_string($headBranch) && $headBranch !== $defaultBranch) return;

    $run = status_lights_app_read($system, StatusLightsAppStoreKind::Runs, (string) $runId);
    $workflow = $run['workflow'] ?? null;
    if (!is_string($workflow)) return;
...
function status_lights_app_handle_webhook(StatusLightsSystem $system, array $server): StatusLightsResponse
{
    ...
    if (!status_lights_app_verify_signature($system, $body, $server)) return status_lights_create_json_response(['error' => 'Invalid webhook signature'], 401);

    try {
        $payload = json_decode($body, true, flags: JSON_THROW_ON_ERROR);
    } catch (JsonException) { return status_lights_create_json_response(['error' => 'Invalid JSON'], 400); }

    if (!is_array($payload)) return status_lights_create_json_response(['error' => 'Invalid payload'], 400);
    ...
    if ($event === null) return status_lights_create_json_response(['error' => 'Invalid webhook event'], 400);

    $deliveryId = status_lights_app_normalize_delivery_id($server['HTTP_X_GITHUB_DELIVERY'] ?? null);
    if ($deliveryId === null) return status_lights_create_json_response(['error' => 'Invalid webhook delivery ID'], 400);

    try {
        $claimed = status_lights_app_claim_delivery($system, $deliveryId);
    } catch (Throwable) { return status_lights_create_json_response(['error' => 'Unable to record webhook delivery'], 500); }

    if (!$claimed) return status_lights_create_json_response(['status' => 'accepted', 'event' => $event, 'duplicate' => true], 202);
    if ($event === 'ping') return status_lights_create_json_response(['status' => 'ok', 'event' => 'ping']);

    try {
        if ($event === 'installation' || $event === 'installation_repositories') status_lights_app_handle_installation($system, $event, $payload);
        elseif ($event === 'workflow_run') status_lights_app_handle_workflow_run($system, $payload);
        elseif ($event === 'workflow_job') status_lights_app_handle_workflow_job($system, $payload);
    } catch (Throwable) {
        try { status_lights_app_release_delivery($system, $deliveryId); } catch (Throwable) {}
...
function status_lights_app_handle_request(StatusLightsSystem $system, array $server): StatusLightsResponse {
    ...
    if ($path === '/webhooks/github') return status_lights_app_handle_webhook($system, $server);
